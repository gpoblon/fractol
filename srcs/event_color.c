...
/*   Updated: 2017/01/07 12:48:39 by gpoblon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_color_preset(int keycode, t_app *app)
{
	if (keycode == 12)
		*app->fractal->cs = (t_cscheme){{0.34, 0.185, 0.15}, {6, 6, 6}, {197,
							210, 210}, {55, 55, 55}, app->fractal->cs->lastkey};
	else if (keycode == 13)
		*app->fractal->cs = (t_cscheme){{0.31, 0.155, 0.21}, {14, 7, 10}, {197,
							180, 230}, {55, 75, 80}, app->fractal->cs->lastkey};
	else if (keycode == 14)
		*app->fractal->cs = (t_cscheme){{0.25, 0.645, 0.14}, {6, 6, 6}, {207,
							210, 190}, {50, 70, 90}, app->fractal->cs->lastkey};
	else if (keycode == 15)
		*app->fractal->cs = (t_cscheme){{0.03, 0.015, 0.01}, {6, 6, 6}, {97,
							200, 200}, {50, 55, 60}, app->fractal->cs->lastkey};
}
...
static void	ft_plus_color(t_app *app)
{
...
static void	ft_minus_color(t_app *app)
{
	if (app->fractal->cs->lastkey & 1 << 0)
		app->fractal->cs->freq[0] -= 0.010;
	else if (app->fractal->cs->lastkey & 1 << 1)
